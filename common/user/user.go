...
func GeneratePasswd(workshop *workshopv1.Workshop, username string) []byte {
	...
	shellScript, err := ioutil.ReadFile("common/user/generate_htpasswd.sh")
	if err != nil {
	...
	if err = ioutil.WriteFile("common/user/generate_htpasswd.sh", shellScript, 0755); err != nil {
		log.Fatal(err)
	}
	_, err = exec.Command("/bin/bash", "common/user/generate_htpasswd.sh").Output()
	if err != nil {
